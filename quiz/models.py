...
class QuestionQuerySet(models.QuerySet):
    def _get_subclass_models(self):
        return [
            r.related_model
            for r in self.model._meta.related_objects
            if r.parent_link
        ]

    def select_subclasses(self, *subclasses):
        if not subclasses:
            subclasses = self._get_subclass_models()
        new_qs = self.select_related(
            *[s._meta.model_name for s in subclasses]
        )
        new_qs._subclasses = subclasses
        return new_qs

    def _clone(self):
        c = super()._clone()
        c._subclasses = getattr(self, '_subclasses', None)
        return c

    def iterator(self, *args, **kwargs):
        subclasses = getattr(self, '_subclasses', None)
        for obj in super().iterator(*args, **kwargs):
            if subclasses:
                for sub in subclasses:
                    child = getattr(obj, sub._meta.model_name, None)
...
class QuestionManager(models.Manager):
    ...
    def get_subclass(self, **kwargs):
        obj = self.get(**kwargs)
        for related in self.model._meta.related_objects:
            if related.parent_link:
                child = getattr(obj, related.related_model._meta.model_name, None)
                if child is not None:
                    return child
        return obj
